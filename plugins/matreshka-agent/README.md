...
Статус `0.1.4`: development preview. Пакет проходит локальную структурную и security-валидацию, но ещё не считается публичным релизом без native installation smoke tests, финальных publisher metadata и иконки.
...
В интерфейсах Codex карточки и плашки навыков версии `0.1.4` используют формат `Название действия · Matreshka Agent`, поэтому сначала видна задача, а затем источник навыка.
...
## Что намеренно не входит в 0.1.4
