...
> Статус: собрана локально проверенная development preview версии `0.4.0`. В пакет входят десять навыков и четыре платформенных манифеста. Offline self-test проходит; нативные установки и publisher metadata ещё требуют отдельной проверки перед публичным релизом.

## Build End-to-End — одна точка входа

Для разработки проекта целиком запустите `building-end-to-end` и опишите результат обычными словами. В Codex можно использовать `$matreshka-agent:building-end-to-end`, меню `/skills` или optional wrapper `/prompts:matreshka-build`; в Claude Code — `/matreshka-agent:building-end-to-end`. Этот skill уточняет только блокирующие продуктовые решения и передаёт работу проверенному controller-у.

Режим взаимодействия, профиль качества и полномочия — разные вещи:

- `GUIDED` ставит подтверждения на переходах;
- `ASSISTED` по умолчанию разрешает обратимые решения внутри локального scope;
- `AUTONOMOUS_LOCAL` уменьшает число пауз, но не разрешает Git, сеть, секреты, provider calls или deploy;
- execution profile определяет глубину тестов и review;
- permission envelope отдельно определяет реально разрешённые действия.

При разрешённой записи агент переиспользует один `CONTEXT.md` или `docs/context.md`, создаёт ADR только для значимых решений в `docs/adr/` и ведёт человекочитаемый `docs/runs/<run-id>/progress.md`. Эти документы не дают полномочий и не являются доказательством готовности: ledger, фактическое состояние репозитория и свежие проверки остаются главнее.
...
2. Для проекта целиком запустите `building-end-to-end`; для ручного управления workflow — `orchestrating-subagent-work`.
...
- quality gate не превращает отсутствующую проверку в warning и не расширяет permissions;
- candidate directed learning не создаётся без режима `PROPOSE`/`LOCAL_REVIEWED` и никогда не попадает в global instructions автоматически;
- worktree не создаётся или не удаляется без явного разрешения и ownership.
...
| --- | --- | --- |
| Project profile | Project-local карта известных команд, ограничений и разрешённых Matreshka skills | Не становится глобальной памятью и не заменяет повторную проверку фактов |
...
В обычной задаче ничего дополнительно включать не нужно. Controller предложит маленький quality gate после read-only preflight и покажет, какие команды он собирается выполнить.

Если вы хотите, чтобы система извлекла полезный урок из результата, добавьте к запросу один из режимов:

```text
Режим обучения: OFF
```

Это значение по умолчанию: никакой candidate не создаётся.

```text
Режим обучения: PROPOSE.
После завершения предложи не более одного project-local урока.
Укажи evidence, применимость и что нужно повторно проверить.
Не сохраняй и не применяй его автоматически.
```

Для заранее согласованного локального использования нужен более строгий режим:

```text
Режим обучения: LOCAL_REVIEWED.
Разрешаю сохранить candidate только в папке проекта.
Применять его можно лишь после моего отдельного одобрения
и независимой повторной проверки в новой подходящей задаче.
```

Controller не включает этот режим сам. Candidate не может содержать секреты, персональные данные, private URLs, raw logs или скрытые рассуждения. Он обязан содержать ссылку на наблюдаемое evidence, narrow applicability и условия, при которых его нужно отбросить. Если проверка не прошла, честный результат — `PARTIALLY_VERIFIED` или `BLOCKED`; «обучение» не превращает неподтверждённое предположение в правило.

### Optional worktree isolation

Для рискованной задачи можно отдельно разрешить isolated worktree. Controller создаёт его только после точного согласия на Git workspace и ownership, не запускает в одном checkout параллельных writers и никогда не удаляет worktree автоматически. Если такая изоляция недоступна, это фиксируется как ограничение профиля, а не скрывается.
...
Инструкции сверены с официальной документацией по состоянию на **18 июля 2026 года**:
